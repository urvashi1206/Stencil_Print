    ...
    public class SolidifyOnContact : MonoBehaviour
    {
        ...
        void Solver_OnCollision(object sender, ObiNativeContactList e)
        {
            ...
            for (int i = 0; i < e.count; ++i)
            {
                ...
                {
                    int handleIndex = e[i].bodyB;

                    // Safety check: bodyB index must be valid
                    if (handleIndex < 0 || handleIndex >= colliderWorld.colliderHandles.Count)
                        continue;

                    var handle = colliderWorld.colliderHandles[handleIndex];
                    ...
                    {
                        int pIndex = solver.simplices[e[i].bodyA];
                        solver.life[pIndex] = 0f;
                        continue;
                    }

                    if (!col.CompareTag("PCB"))
                    {
                        continue;
                    }
        ...
        {
            //Debug.Log($"Solidified particle {particleIndex} on contact with {solid.reference.name}");
            // remove the 'fluid' flag from the particle, turning it into a solid granule:
            if (particleIndex < 0 || particleIndex >= solver.allocParticleCount)
                return;

            if (solids.Length <= particleIndex)
                Array.Resize(ref solids, solver.allocParticleCount);
